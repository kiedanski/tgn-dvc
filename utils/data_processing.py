...
#   graph_df["ts"] = graph_df["ts"] * 100
#   edge_features = np.load('./data/ml_{}.npy'.format(dataset_name))
#   node_features = np.load('./data/ml_{}_node.npy'.format(dataset_name))

#   edge_features = np.random.uniform(1, 2, size=edge_features.shape)
#   node_features = np.random.uniform(1, 2, size=node_features.shape)

#   m, M = edge_features.min(), edge_features.max()
#   edge_features = (edge_features - m) / (M - m)

#   m, M = node_features.min(), node_features.max()
#   node_features = (node_features - m) / (M - m)
#   print(np.median(node_features))
#   print(np.median(edge_features))
